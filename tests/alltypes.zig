...
test "TestExtremeDefaultValues" {
    ...
    try testing.expectEqualSlices(u8, "\xE1\x88\xB4", decoded.utf8_string.?.getSlice());
    ...
    try testing.expectEqualSlices(u8, "? ? ?? ?? ??? ??/ ??-", decoded.cpp_trigraph.?.getSlice());
    try testing.expectEqualSlices(u8, "hel\x00lo", decoded.string_with_zero.?.getSlice());
    try testing.expectEqualSlices(u8, "wor\\000ld", decoded.bytes_with_zero.?.getSlice());
    try testing.expectEqualSlices(u8, "ab\x00c", decoded.string_piece_with_zero.?.getSlice());
    try testing.expectEqualSlices(u8, "12\x003", decoded.cord_with_zero.?.getSlice());
    try testing.expectEqualSlices(u8, "${unknown}", decoded.replacement_string.?.getSlice());
}
