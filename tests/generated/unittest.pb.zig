...
pub fn TestService(errorType: type) type {
    return struct {
        const Interface = struct {
            Foo: *const fn (x: *anyopaque, p: FooRequest) errorType!FooResponse,
            Bar: *const fn (x: *anyopaque, p: BarRequest) errorType!BarResponse,
        };

        _ptr: *anyopaque,
        _vtab: *const Interface,

        pub fn init(obj: anytype) TestService(errorType) {
            ...
            const impl = struct {
                pub fn Foo(ptr: *anyopaque, param: FooRequest) errorType!FooResponse {
                    const self: Ptr = @ptrCast(@alignCast(ptr));
                    return self.Foo(param);
                }
                pub fn Bar(ptr: *anyopaque, param: BarRequest) errorType!BarResponse {
                    const self: Ptr = @ptrCast(@alignCast(ptr));
        ...
        pub fn Foo(self: TestService(errorType), param: FooRequest) errorType!FooResponse {
            return self._vtab.Foo(self._ptr, param);
        }

        pub fn Bar(self: TestService(errorType), param: BarRequest) errorType!BarResponse {
